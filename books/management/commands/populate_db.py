...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        authors = (Author(name=f"Author {i}", age=randint(20, 80)) for i in range(1, 71))
        Author.objects.bulk_create(authors)

        publishers = (Publisher(name=f"Publisher {i}") for i in range(1, 31))
        ...
        stores = (Store(name=f"Store {i}") for i in range(1, 11))
